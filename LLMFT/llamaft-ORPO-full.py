...
base_model = "meta-llama/Meta-Llama-3.1-8B" #meta-llama/Meta-Llama-3.1-8B-Instruct
		...
		if i.split('.')[-1]=='pkl':
			with open(dir+i, 'rb') as f:
				...
				for i in data:
					...
					datum = {'instruction': INSTRUCTION, 'input': i['prompt'], 'output': target_datum}
...
def get_data_orpo(train_dict):
    ds_chat = [{'chosen': [{'content': INSTRUCTION+'\n'+i['input']+QU, 'role': 'user'}, {'content': i['output'], 'role': 'assistant'}], 
                ...
                'prompt': INSTRUCTION+i['input']+QU, 'output': i['output']} for i in train_dict]
...
dataset_test = get_data_orpo(test_dict[:500])
...
print('saving the model')
res, (real_output_filtered, generated_output_filtered) = evaluator(model,tokenizer, dataset_test)
